 ...
 */
class RovrREIEDI extends SPSCommerceBaseClient{
       ...
       /* if (!dropShipOrder) {
        ...
        if (!dropShipOrder) {
            if(order.shippingInfo.carr_service.equals("LTL")){
                order.injectWorkOrder("ROVR/REI LTL work order OrderNum: " + order.orderNum ,
                        "Please help prepare this LTL order for shipping. Please provide pallet count, dims and weight so we can schedule the pick up.\n\n" +
                        "***Please stack 6 cartons per pallet***\n" +
                        "***MUST PRINT AND APPLY UCC-128 LABLES FROM OWD SYSTEM\n\n" +
                        "OWD#" + order.orderNum +
