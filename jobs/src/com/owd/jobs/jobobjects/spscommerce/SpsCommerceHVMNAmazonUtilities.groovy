...
class SpsCommerceHVMNAmazonUtilities {
    ...
    def static int saveIncomingEdiDoc(String docContent, String fileName, boolean testing) {
        ...
        println(fileName)
        println(testing)
        PreparedStatement ps = HibernateSession.getPreparedStatement("insert into edi_docs (docData,sourceFile,docStatus) values (?,?,?);SELECT @@IDENTITY")
    ...
    def static deliverAsn(SPSCommerceRemoteFTP ftp, int orderId, int clientId) {
        ...
        String asnData = SpsCommerceHVMNAmazonUtilities.generateASN(orderId, clientId)
        println asnData
    ...
    def static void main(String[] args) {
       ...
       // System.out.println(getUPCFromOwdSku(configdata, "P222189"))
        String asnData = SpsCommerceHVMNAmazonUtilities.generateASN(17210785,576);
        println asnData;
    ...
    public static void processPendingPos() {
        try {
            List<EdiDocs> docsToProcess = HibernateSession.currentSession().createQuery("from EdiDocs where docStatus=0").list()
            for (EdiDocs doc : docsToProcess) {
                ...
                try {
                    ...
                    HibUtils.commit(HibernateSession.currentSession())
                    //   HibernateSession.currentSession().flush()
                    EdiSpsConfigdata config = SpsCommerceHVMNAmazonUtilities.getEdiConfigData(po)
                    ...
                    if (clientId == 489 || clientId == 491) {  //Symphony
                        ...
                        if (config.getSpsaccount().equals("080ALLGILDANUSA")) {
                    ...
                    HibUtils.commit(HibernateSession.currentSession())
                    try {
                        if (config.acknowledgmentRequired == 1) {
                            ...
                            String ack = SpsCommerceHVMNAmazonUtilities.generateACK(config, po, Integer.parseInt(order.clientID), oorder)
                            println ack
                        ...
                        throw new LogableException("Unable to properly Acknoledge: " + e.getMessage(), order.orderNum, clientId + "", "EDI ACK", LogableException.errorTypes.ASN_IMPORT);
                    }
    ...
    def static String getOwdSkuFromForeignSku(EdiSpsConfigdata config, String skuValue){
        String sku = null;
    ...
    def static String getForeignSkuFromOWDSku(EdiSpsConfigdata config, String skuValue){
        String sku = null;
       // println("select owd_sku from owd_inventory_sku_maps  where client_fkey=" + config.getClientFkey() + " and foreign_sku='" + skuValue.trim() + "'" + " and vendor_compliance_fkey='" + config.getVendorComplianceFkey() + "'")

        ResultSet rs = HibernateSession.getResultSet("select foreign_sku from owd_inventory_sku_maps  where client_fkey=" + config.getClientFkey() + " and owd_sku ='" + skuValue.trim() + "'" + " and vendor_compliance_fkey='" + config.getVendorComplianceFkey() + "'")
        if (rs.next()) {
            sku = rs.getString(1)
        }
        rs.close()
    ...
    def static Order importPo(Node podoc, int clientID) throws Exception {
        ...
        def lines = podoc.LineItems.LineItem.OrderLine.each {
            ...
            if((configdata.spsaccount.equals("FYAALLHVMNINC00"))){
                internalSku = getOwdSkuFromForeignSku(configdata,it.BuyerPartNumber.text())
                ...
                itemDescription = ""
                    ...
                    //check for stock and only ship what we have
                        ...
                        if(stock == 0){
                            ...
                            lie.setVendor_sku(it.BuyerPartNumber.text())
                            ...
                            int qty = Integer.parseInt(it.OrderQty.text());

                            Float price = Float.parseFloat(it.PurchasePrice?.text());
                            price = price/owdInv.caseQty;


                            println("qty: "+qty);

                            if(owdInv.caseQty>0){
                                println("Case Qty: "+ owdInv.caseQty);
                                int availableCases = stock/owdInv.caseQty;
                                println(availableCases);
                                if(availableCases<qty){
                                    ...
                                    if(availableCases==0){
                                        ...
                                        println("Adding partial: " + availableCases * owdInv.caseQty);
                                        order.addLineItem(internalSku, "" + availableCases * owdInv.caseQty, price + "", "0.00", itemDescription, "", "")
                                        def lie = new lineItemExemptions()
                                        ...
                                        lie.setExemptionCode("quantity");
                                        lie.setExemptionValue("short ship");
                                        lie.setQty(((qty - availableCases) * owdInv.caseQty) + "")
                                        lie.setMerchant_line_number(it.LineSequenceNumber.text())
                                    ...
                                    // WE have enough
                                    println("Adding full amount: "+  qty*owdInv.caseQty)
                                    order.addLineItem(internalSku, "" + qty*owdInv.caseQty, price+"", "0.00", itemDescription, "", "")
                    ...
                    Shipment {
                        ...
                        Header {
                            ShipmentHeader {
                                ...
                                com.owd.core.business.order.Package pack = com.owd.core.business.order.Package.getPackagesForOrderId(orderId).get(0);
                                ...
                                if (configdata.spsaccount.equals("080ALLGILDANUSA")||configdata.spsaccount.equals("FYAALLHVMNINC00")) {
                                    ...
                                    BillOfLadingNumber(order.orderNum);
                            ...
                            */
                            if(scacCode.length()==0){
                                scacCode = poData.Header.CarrierInformation.CarrierAlphaCode.text().trim()
                            }
                            ...
                            com.owd.core.business.order.Package.getPackagesForOrderId(orderId).each { pack ->
                                ...
                                PackLevel {
                                    ...
                                    if (configdata.spsaccount.equals("FYAALLHVMNINC00")) {
                                        ...
                                        List<lineItemPackageData> packData = Package.getLineItemDataMapforPackage(Integer.parseInt(pack.order_track_id));
                                        ...
                                        items.each { itemMap ->
                                            ItemLevel {
                                                ...
                                                if (vendorSku == null || vendorSku.length() < 1) {
                                                    ...
                                                    String fSku = SpsCommerceHVMNAmazonUtilities.getForeignSkuFromOWDSku(configdata, "" + itemMap.get('SKU'))
                                                    println "got fSku " + fSku + " for " + itemMap.get('SKU')
                                                    def poLineItem = poData.LineItems.LineItem.OrderLine.find {
                                                        println "checking BuyerPartNumber " + it.BuyerPartNumber.text()
                                                        it.BuyerPartNumber.text().trim().equals(fSku)
                                                    }


                                                    if (poLineItem) {
                                                        def currentPData = new lineItemPackageData();

                                                        for( pData in packData){
                                                            if(pData.getSku().equalsIgnoreCase(itemMap.get('SKU'))){
                                                                currentPData = pData;
                                                            }
                                                        ...
                                                        ShipmentLine {
                                                            ...
                                                            ConsumerPackageCode(poLineItem.ConsumerPackageCode ? poLineItem.ConsumerPackageCode.text().trim() : fSku)

                                                            if(currentPData.getLots().size()>0){
                                                               def ld = currentPData.getLots().get(0);

                                                                PartNumberQualifier3('LT');
                                                                PartNumber3(ld.getLotValue())

                                                            }
                                                            if("CA".equals(poLineItem.OrderQtyUOM?.text().trim())){
                                                                OwdInventory inv =   InventoryManager.getOwdInventoryForClientAndSku(clientId+"",itemMap.get('SKU'));

                                                                ShipQty(Integer.parseInt(''+itemMap.get('QTY'))/inv.caseQty + '.0')
                                                                totalQuantity += Integer.parseInt('' + itemMap.get('QTY'))/inv.caseQty
                                                            }else{
                                        ...
                                        ItemLevel {
                                            ...
                                            items.each { itemMap ->
                                                ...
                                                println vendorSku
                                                if (vendorSku == null || vendorSku.length() < 1) {
                                                    String upc = SpsCommerceHVMNAmazonUtilities.getUPCFromOwdSku(configdata, "" + itemMap.get('SKU'))
                            ...
                            poData.LineItems.LineItem.OrderLine.each { line ->
                                ...
                                LineItem {
                                    ...
                                    LineItemAcknowledgement {
                                        ...
                                        if(exemptLines.containsKey(line.BuyerPartNumber.text())){
                                           def lie = exemptLines.get(line.BuyerPartNumber.text())
                                            //determine type of exemption
                                            if(lie.exemptionCode.equals("quantity")&&lie.exemptionValue.equals("short ship")){
                                                ItemStatusCode("IQ")
                                                if(line.OrderQtyUOM.text().equals("CA")){
                                                 OwdInventory inv =   InventoryManager.getOwdInventoryForClientAndSku(clientId+"",lie.getInventory_num());

                                                totalQuantity +=Integer.parseInt(line.OrderQty.text())-(Integer.parseInt(lie.qty)/inv.getCaseQty())
                                                    ItemScheduleQty(Integer.parseInt(line.OrderQty.text())-(Integer.parseInt(lie.qty)/inv.getCaseQty()))

                                                }else{
                                                    ItemScheduleQty(Integer.parseInt(line.OrderQty.text())-Integer.parseInt(lie.qty))
                                                    totalQeuty += Integer.parseInt(line.OrderQty.text())-Integer.parseInt(lie.qty);

                                                }

                                               ItemScheduleQualifier("068")
                                                ItemScheduleUOM(line.OrderQtyUOM.text())
                                        ...
                                        ItemStatusCode("IA")
                                        ItemScheduleQty(''+line.OrderQty.text())
                                          totalQuantity +=  Integer.parseInt(line.OrderQty.text());
