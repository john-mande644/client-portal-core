 ...
 */
public class RHLReportDefinition extends MonthlyReportDefinition {



    public RHLReportDefinition(Date firstDatePastPeriod, Date firstDateOfPreviousMonth) {

        super(firstDatePastPeriod,firstDateOfPreviousMonth);
    }

    public RHLReportDefinition() {
        
        super();


        clientName = "Real Health Labs";
        SheetDefinition sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetTitle="RealHealthLabs.com Order Source Financial Report" ;
        sheet.sheetName="RHL Orders Report" ;
        sheet.query=  "SELECT\n" +
                "                    dbo.udf_getcherryordertype(order_id) AS 'Order Type',\n" +
                ...
                "                AND post_date<'" + df.format(firstDatePastPeriod) +"' AND dbo.udf_getsambucolFlag(order_id)=0 and dbo.udf_getBioglanFlag(order_id)=0\n" +
                "                GROUP BY\n" +
                "                   dbo.udf_getcherryordertype(order_id) ";
        sheets.add(sheet);
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetName="RHL SKU Report" ;
        sheet.sheetTitle="RealHealthLabs.com SKU-based Financial Report" ;
        sheet.query=  "SELECT\n" +
                ...
                "WHERE\n" +
                "    i.client_fkey in ("+sheet.clientFkey+") and i.inventory_num not in ('PPS','MAIL') and  charindex('80',i.inventory_num)<>1 AND charindex('40',i.inventory_num)<>1 AND charindex('SAM',i.inventory_num)<>1\n" +
                "GROUP BY\n" +
                "    i.inventory_num,\n" +
                "    i.price,\n" +
                "    supp_cost,\n" +
        ...
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetName="RHL Inventory Count" ;
        sheet.sheetTitle="RealHealthLabs.com Inventory Levels and Transactions" ;
        sheet.query=  "select SKU,max(descr) as Title,max(startcnt) as 'Start Count',sum(a) as Damaged, sum(b) as 'Shipped', sum(c) as 'Returned',sum(d) as 'Received',sum(e) as 'Adjusted',max(endcnt) as 'End Count' from\n" +
                ...
                "                      and i.inventory_num not in ('Mail','PPS') and is_auto_inventory=0 and  charindex('80',i.inventory_num)<1 AND charindex('40',i.inventory_num)<1 AND charindex('SAM',i.inventory_num)<1\n" +
                ...
                "                     and i.inventory_num not in ('Mail','PPS') and is_auto_inventory=0 and i.inventory_num not in ('PPS','MAIL') and  charindex('80',i.inventory_num)<1 AND charindex('40',i.inventory_num)<1 AND charindex('SAM',i.inventory_num)<1\n" +
        ...
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetTitle="Sambucol Order Source Financial Report" ;
        sheet.sheetName="SAM Orders Report" ;
        sheet.query=  "SELECT\n" +
                "                    dbo.udf_getcherryordertype(order_id) AS 'Order Type',\n" +
                "                    COUNT(DISTINCT(order_id))            AS 'Shipped Orders',\n" +
                "                    SUM(shipped_units)                   AS 'Shipped Units',\n" +
                ...
                "                    client_fkey in ("+sheet.clientFkey+")\n" +
                "                AND  post_date>='" + df.format(firstDateOfPreviousMonth) + "'\n" +
                "                AND post_date<'" + df.format(firstDatePastPeriod) +"' AND dbo.udf_getsambucolFlag(order_id)=1\n" +
                "                GROUP BY\n" +
                "                   dbo.udf_getcherryordertype(order_id) ";
        sheets.add(sheet);
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetName="SAM SKU Report" ;
        sheet.sheetTitle="Sambucol SKU-based Financial Report" ;
        sheet.query=  "SELECT\n" +
                "    i.inventory_num                AS 'SKU',\n" +
                "    ISNULL(SUM(quantity_actual),0) AS 'Shipped Units',\n" +
                "    ISNULL(supp_cost,0.0000)       AS 'Unit Cost',\n" +
                ...
                "    i.inventory_id=l.inventory_id\n" +
                "WHERE\n" +
                "    i.client_fkey in ("+sheet.clientFkey+") and i.inventory_num not in ('PPS','MAIL') and  charindex('SAM',i.inventory_num)=1\n" +
                "GROUP BY\n" +
                "    i.inventory_num,\n" +
                "    i.price,\n" +
                "    supp_cost,\n" +
        ...
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetName="SAM Inventory Count" ;
        sheet.sheetTitle="Sambucol Inventory Levels and Transactions" ;
        sheet.query=  "select SKU,max(descr) as Title,max(startcnt) as 'Start Count',sum(a) as Damaged, sum(b) as 'Shipped', sum(c) as 'Returned',sum(d) as 'Received',sum(e) as 'Adjusted',max(endcnt) as 'End Count' from\n" +
                "(select sku,(descr) as descr,ISNULL(startcnt,0) as startcnt,ISNULL(endcnt,0) as endcnt,\n" +
                "case when type='Damaged' then count else 0 end as a,\n" +
                "case when type='Posted Order' then count else 0 end as b,\n" +
                ...
                "                      where v.client_fkey in ("+sheet.clientFkey+") and\n" +
                "                      transactiondate>='" + df.format(firstDateOfPreviousMonth) + "' and transactiondate<'" + df.format(firstDatePastPeriod) + "'\n" +
                "                      and i.inventory_num not in ('Mail','PPS') and is_auto_inventory=0 and  charindex('SAM',i.inventory_num)=1\n" +
                "                      group by qty_on_hand,i.inventory_id,i.client_fkey,i.description,transactiontype, i.inventory_num) as t\n" +
                "left outer join\n" +
                "\n" +
                "(select  i.inventory_num, i.inventory_id as iid,ISNULL(case when (qty_on_hand+sum(-1*ISNULL(quantitychange,0)))<0 then 0 else (qty_on_hand+sum(-1*ISNULL(quantitychange,0))) end,qty_on_hand)  as 'startcnt',i.client_fkey as cid\n" +
                ...
                "join owd_inventory_oh h on h.inventory_fkey=i.inventory_id\n" +
                "                     where i.client_fkey in ("+sheet.clientFkey+")\n" +
                "                     and i.inventory_num not in ('Mail','PPS') and is_auto_inventory=0 and i.inventory_num not in ('PPS','MAIL') and  charindex('SAM',i.inventory_num)=1\n" +
                "                     group by qty_on_hand,i.inventory_num,i.inventory_id,i.client_fkey,i.description) as c1\n" +
                "\n" +
                "on c1.iid=t.iid\n" +
                "left outer join\n" +
        ...
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetTitle="Bioglan Order Source Financial Report" ;
        sheet.sheetName="Bioglan Orders Report" ;
        sheet.query=  "SELECT\n" +
                "                    dbo.udf_getcherryordertype(order_id) AS 'Order Type',\n" +
                "                    COUNT(DISTINCT(order_id))            AS 'Shipped Orders',\n" +
                "                    SUM(shipped_units)                   AS 'Shipped Units',\n" +
                ...
                "                    client_fkey in ("+sheet.clientFkey+")\n" +
                "                AND  post_date>='" + df.format(firstDateOfPreviousMonth) + "'\n" +
                "                AND post_date<'" + df.format(firstDatePastPeriod) +"' AND dbo.udf_getBioglanFlag(order_id)=1\n" +
                "                GROUP BY\n" +
                "                   dbo.udf_getcherryordertype(order_id) ";
        sheets.add(sheet);
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetName="Bioglan SKU Report" ;
        sheet.sheetTitle="Bioglan SKU-based Financial Report" ;
        sheet.query=  "SELECT\n" +
                "    i.inventory_num                AS 'SKU',\n" +
                "    ISNULL(SUM(quantity_actual),0) AS 'Shipped Units',\n" +
                "    ISNULL(supp_cost,0.0000)       AS 'Unit Cost',\n" +
                ...
                "    i.inventory_id=l.inventory_id\n" +
                "WHERE\n" +
                "    i.client_fkey in ("+sheet.clientFkey+") and i.inventory_num not in ('PPS','MAIL') and  (charindex('80',i.inventory_num)=1 or charindex('40',i.inventory_num)=1)\n" +
                "GROUP BY\n" +
                "    i.inventory_num,\n" +
                "    i.price,\n" +
                "    supp_cost,\n" +
        ...
        sheet = new SheetDefinition();
        sheet.clientFkey=401;
        sheet.sheetName="Bioglan Inventory Count" ;
        sheet.sheetTitle="Bioglan Inventory Levels and Transactions" ;
        sheet.query=  "select SKU,max(descr) as Title,max(startcnt) as 'Start Count',sum(a) as Damaged, sum(b) as 'Shipped', sum(c) as 'Returned',sum(d) as 'Received',sum(e) as 'Adjusted',max(endcnt) as 'End Count' from\n" +
                "(select sku,(descr) as descr,ISNULL(startcnt,0) as startcnt,ISNULL(endcnt,0) as endcnt,\n" +
                "case when type='Damaged' then count else 0 end as a,\n" +
                "case when type='Posted Order' then count else 0 end as b,\n" +
                ...
                "                      where v.client_fkey in ("+sheet.clientFkey+") and\n" +
                "                      transactiondate>='" + df.format(firstDateOfPreviousMonth) + "' and transactiondate<'" + df.format(firstDatePastPeriod) + "'\n" +
                "                      and i.inventory_num not in ('Mail','PPS') and is_auto_inventory=0 and   (charindex('80',i.inventory_num)=1 or charindex('40',i.inventory_num)=1)\n" +
                "                      group by qty_on_hand,i.inventory_id,i.client_fkey,i.description,transactiontype, i.inventory_num) as t\n" +
                "left outer join\n" +
                "\n" +
                "(select  i.inventory_num, i.inventory_id as iid,ISNULL(case when (qty_on_hand+sum(-1*ISNULL(quantitychange,0)))<0 then 0 else (qty_on_hand+sum(-1*ISNULL(quantitychange,0))) end,qty_on_hand)  as 'startcnt',i.client_fkey as cid\n" +
                ...
                "join owd_inventory_oh h on h.inventory_fkey=i.inventory_id\n" +
                "                     where i.client_fkey in ("+sheet.clientFkey+")\n" +
                "                     and i.inventory_num not in ('Mail','PPS') and is_auto_inventory=0 and i.inventory_num not in ('PPS','MAIL') and   (charindex('80',i.inventory_num)=1 or charindex('40',i.inventory_num)=1)\n" +
                "                     group by qty_on_hand,i.inventory_num,i.inventory_id,i.client_fkey,i.description) as c1\n" +
                "\n" +
                "on c1.iid=t.iid\n" +
                "left outer join\n" +
