 ...
 */
public class RenuInfusionUploadData extends OWDUploadOrdersData_1{
   ...
   public static int ShipPostalCode = 14;
       ...
       public Order loadOrder(Order order, int orderIndex) throws Exception {
        ...
        for (int row = data.rowIndexStart; row < rdr.getRowCount(); row++) {
            ...
            if (row == data.rowIndexStart) {
                //first row
                if("266".equals(order.clientID))
                {
                    order.order_refnum = rdr.getStrValue(Id, row, "")+"-BC";
                }   else{
                ...
                if((!("375".equals(order.clientID))) && OrderUtilities.orderRefnumExists(order.order_refnum, order.clientID))
                {
                    throw new Exception("Duplicate order reference; already imported");
                }
                ...
                order.getBillingAddress().country = rdr.getStrValue(Country, row, "USA");
                ...
                order.getShippingAddress().zip = rdr.getStrValue(PostalCode, row, "");
                    order.getShippingAddress().country = rdr.getStrValue(Country, row, "USA");
                ...
                order.getShippingAddress().zip = rdr.getStrValue(ShipPostalCode, row, "");
                    order.getShippingAddress().country = rdr.getStrValue(ShipCountry, row, "USA");
                }


                order.getShippingAddress().country =  order.getShippingAddress().country.toUpperCase().replaceAll(", SWISS CONFEDERATION","");
                order.getBillingAddress().country =  order.getBillingAddress().country.toUpperCase().replaceAll(", SWISS CONFEDERATION","");
                ...
                if (order.getBillingAddress().country.equalsIgnoreCase("CHINA"))
                {
                    order.getBillingAddress().country = "CHINA PEOPLES REPUBLIC OF";
                }

                if (order.getBillingAddress().country.equalsIgnoreCase("KOREA"))
                {
                    order.getBillingAddress().country = "KOREA SOUTH";
                }
            ...
            if((("IS-"+rdr.getStrValue(Id, row, "")).equals(order.order_refnum)) ||
                    ((rdr.getStrValue(Id, row, "")+"-IS").equals(order.order_refnum)))
            {
                itemIDTotal += rdr.getIntValue(16,row,0);
            //product data collected for every row
                String SKU = rdr.getStrValue(Sku,row,"").trim();
                if("Triple Power Fish Oil 3 Month Supply Single".equals(SKU))
                {
                    SKU = "Triple Power Fish Oil 3 Month Supply";
                }
...

    {
     transMap = new TreeMap();
                ...
                transMap.put("Regular","Economy");
                transMap.put("Priority","Ground");
        transMap.put("Shipping","Ground");
                transMap.put("Express","Standard Priority");
     ...
     public String unmapShip(String method) throws Exception{
        ...
        }   else if(method.toUpperCase().contains("RUSH"))
         {
