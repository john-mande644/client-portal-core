    ...
    public static void invokeGetFeedSubmissionResult(MarketplaceWebService service, List<GetFeedSubmissionResultRequest> requests) {
        ...
        for (Future<GetFeedSubmissionResultResponse> future : responses) {
            ...
            try {
                ...
                log.debug("Result md5checksum : " + response.getGetFeedSubmissionResultResult().getMD5Checksum());
                log.debug("Response request id: " + response.getResponseMetadata().getRequestId());
                log.debug("FeedSubmissionResult: " );
                log.debug( requests.get( responses.indexOf(future)).getFeedSubmissionResultOutputStream().toString());
