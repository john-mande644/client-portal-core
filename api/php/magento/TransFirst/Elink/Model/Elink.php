 ...
 * @category	TransFirst
 * @package		Elink
 * @author		Stewart Buskirk (owditadmin@owd.com)
 * @license		http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 *
	 * @notes		Partially derived from the Fontis SecurePay module, Copyright (c) 2008 Fontis Pty. Ltd. (http://www.fontis.com.au) Also derived from SecurePay XML module by Andrew Dubbeld (support@securepay.com.au)
 */

require_once('TransFirst/transfirst_elink_api.php');
 
/**
 * TransFirst_Elink_Model_Elink
 *
 * The bulk of the TransFirst Elink API payment module. It handles Preauth/Advice, Standard, Reverse and Refund transactions in the Magento application.
 */
class TransFirst_Elink_Model_Elink extends Mage_Payment_Model_Method_Cc
{	
    protected $_code  = 'Elink';
	...
	protected $_formBlockType = 'TransFirst_Elink_block_form_cc';
	...
	{
		return Mage::getStoreConfig('payment/Elink/debug');
	}
	
	public function getMode()
	{
		if(Mage::getStoreConfig('payment/Elink/test'))
		{
	...
	{
		return Mage::getBaseDir() . '/var/log/Elink.log';
	}
	
	public function getUsername()
	{
		return Mage::getStoreConfig('payment/Elink/username');
	}
	
	public function getPassword()
	{
		return Mage::getStoreConfig('payment/Elink/password');
	}
	
	public function getCCVStatus()
	{
		return Mage::getStoreConfig('payment/Elink/usecvv');
	}
	 ...
	 * authorize
	 *
	 * Sends a preauth transaction to the TransFirst ELink Gateway. Only called in "Authorize" mode (See module settings).
	 *
	 * @param Varien_Object $payment
	 * @param int/float $amount
	 *
	...
	public function authorize(Varien_Object $payment, $amount)
	{
		...
		$sxml = new transfirst_elink_transaction ($this->getMode(),trim($this->getUsername()),trim($this->getPassword()));
	...
	public function capture(Varien_Object $payment, $amount)
	{
		...
		if(!$preauth)
		{
			...
			else
			{
				if($this->getDebug())
				{
					$logger->info( "TransFirst_Elink_Model->capture(): CC details are missing in 'Preauth + Capture'. This should not happen.");
				}
		...
		//Create the transaction object
		$sxml = new transfirst_elink_transaction ($this->getMode(),$this->getUsername(),$this->getPassword());
