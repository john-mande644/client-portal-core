 ...
 * transfirst_elink_api.php:

 *

 * Contains a class for sending transaction requests to TransFirst,

 * and receiving responses via the eLink API
...
define( 'TRANSFIRST_GATEWAY_MODE_TEST',			 1);

define( 'TRANSFIRST_GATEWAY_MODE_LIVE',			 2);
...
define( 'TRANSFIRST_TXN_STANDARD',		 0);

define( 'TRANSFIRST_TXN_REFUND',			 4);

define( 'TRANSFIRST_TXN_REVERSE',		 6);

define( 'TRANSFIRST_TXN_PREAUTH', 		10); //To be included in a future revision

define( 'TRANSFIRST_TXN_ADVICE', 		11);



define( 'TRANSFIRST_TXN_DIRECTDEBIT',	15); //To be included in a future revision

define( 'TRANSFIRST_TXN_DIRECTCREDIT', 	17);



define( 'TRANSFIRST_TXN_IVR',		 	20);



define( 'TRANSFIRST_REQ_ECHO', 		  'Echo'); //To be included in a future revision

define( 'TRANSFIRST_REQ_PAYMENT',   'Payment');

define( 'TRANSFIRST_REQ_PERIODIC', 'Periodic'); //To be included in a future revision
 ...
 * transfirst_elink_transaction

 *

 * This class handles ELink TransFirst transactions
 ...
 * @param int mode - The kind of transaction object you would like to open. i.e. TRANSFIRST_GATEWAY_MODE_TEST See top of this file for definitions.
...
class transfirst_elink_transaction

{


    const TRANSFIRST_AUTHORIZE_HOST kAuthorizeURL = "https://epaysecure1.transfirst.com/elink/authpd.asp";

    const TRANSFIRST_AUTHORIZEREVERSAL_HOST kAuthReversalURL = "https://epaysecure1.transfirst.com/elink/authpd.asp";

    const TRANSFIRST_SETTLEMENT_HOST kSettlementURL = "https://epaysecure1.transfirst.com/elink/settlepd.asp";

    const TRANSFIRST_VOID_HOST kVoidURL = "https://epaysecure1.transfirst.com/elink/voidpd.asp";

    const TRANSFIRST_CREDIT_HOST kCreditURL = "https://epaysecure1.transfirst.com/elink/creditpd.asp";

    const TRANSFIRST_ECHECK_HOST kECheckURL = "https://epaysecure1.transfirst.com/elink/checkpd.asp";

    const TRANSFIRST_ECHECKCREDIT_HOST kECheckReturnURL = "https://epaysecure1.transfirst.com/elink/checkcreditPD.asp";
	...
	const GATEWAY_ERROR_TRANSFIRST_STATUS = "The remote Gateway reported the following status error";
...

	{
...

		{
			...
			case TRANSFIRST_GATEWAY_MODE_TEST:

				$this->gatewayURL = self::TRANSFIRST_TEST_HOST;

				break;



			case TRANSFIRST_GATEWAY_MODE_LIVE:

				$this->gatewayURL = self::TRANSFIRST_LIVE_HOST;
...

	{
		...
		$this->setTxnType(TRANSFIRST_TXN_STANDARD);
...

	{
		...
		$this->setTxnType(TRANSFIRST_TXN_REFUND);
...

	{
		...
		$this->setTxnType(TRANSFIRST_TXN_REVERSE);
...

	{
		...
		$this->setTxnType(TRANSFIRST_TXN_PREAUTH);
...

	{
		...
		$this->setTxnType(TRANSFIRST_TXN_ADVICE);
...

	{
		...
		$this->setTxnType(TRANSFIRST_TXN_DIRECTCREDIT);
...

	{
		...
		$this->setTxnType(TRANSFIRST_TXN_DIRECTDEBIT);
	...
	*/
...

	{
		...
		if( $this->getTxnType()==TRANSFIRST_TXN_STANDARD ||

			$this->getTxnType()==TRANSFIRST_TXN_PREAUTH )
		...
		else if ( $this->getTxnType()==TRANSFIRST_TXN_DIRECTDEBIT ||

				  $this->getTxnType()==TRANSFIRST_TXN_DIRECTCREDIT )
...

	{
		...
		if ( $this->getTxnType()==TRANSFIRST_TXN_DIRECTDEBIT ||
...

	{
		...
		"<SecurePayMessage>" .
			...
			if(	$this->getTxnType()==TRANSFIRST_TXN_ADVICE)
			...
			if(	$this->getTxnType()==TRANSFIRST_TXN_REFUND ||

				$this->getTxnType()==TRANSFIRST_TXN_REVERSE &&
	...
	*/
...

	{
...

		{
			...
			$this->errorString = self::GATEWAY_ERROR_TRANSFIRST_STATUS.": ".$statusCode." ".$statusDescription;
		...
		if( $this->getTxnType()==TRANSFIRST_TXN_PREAUTH && array_key_exists('preauthID',$xmlres['SecurePayMessage']['Payment']['TxnList']['Txn']))
		...
		if($this->getRequestType() == TRANSFIRST_REQ_PERIODIC)

		{
...

			{
				...
				($this->getTxnType()==TRANSFIRST_TXN_DIRECTDEBIT ||
		...
		else if ($this->getRequestType() == TRANSFIRST_REQ_PAYMENT)
