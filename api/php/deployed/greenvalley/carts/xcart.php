...
//replace/update with valid ship method codes and translations to OWD ship method codes
		private $ship_type = array(
         ...
         50 => "POS.EXP",
         52 => "OWD.1ST.PRIORITY",
         57 => "OWD_USPS_I_EXP_DMND",
         76 => "FDX.IPR",
         77 => "FDX.IEC",
         78 => "TANDATA_FEDEXGROUND.FEDEX.CAN",
         110 => "UPS.STDCAMX",
         144 => "TANDATA_UPS.UPS.WEXPSVR",
         155 => "OWD_USPS_I_PRIORITY"
		);

	function __construct() {
		//replace below with applicable local database connection info
		$sql_host = 'greenvalleyshop.db.3777873.hostedresource.com';
		$sql_user = 'greenvalleyshop';
		$sql_db = 'greenvalleyshop';
		$sql_password = 'Jasm1ne!+';
...
public function tester()
{
	echo "[TEST]<br>\n";
		...
		while ( $row = @mysql_fetch_assoc ( $rs1 ) ) {
			echo $row['orderid']."=".$row['status']."<br>\n";
			echo $row['b_lastname'];
			print_r($row);
		}
			echo "[TEST-END]<br>\n";
	...
	public function process_orders($id, $code, $testing = false, $redirect = null) {
		echo "[START]<br>\n";
		$query = "SELECT * FROM " . $this->sql_tbl ['orders'] . " WHERE status='p' and orderid > 0 LIMIT 1000";
		$rs1 = mysql_query ( $query );
		$c = 0;
		while ( $row = @mysql_fetch_assoc ( $rs1 ) ) {
				//echo $row['orderid']."=".$row['status']."=".$row['shippingid']."<br>\n";
			if($row['shippingid'] >= 0){
			//	echo $row['shippingid']."<BR>\n";
			...
			//	echo "got line ".$line['productcode']."<BR>\n";
            // $line_items [] = new lineItem ( $line['productcode'], $line['product'], $line['amount'], $line['amount']*$line['price'], $line['amount']*$line['price'], NULL, NULL, ++$count );
			...
			$billing->setLast_name ( $row['lastname'] );
			$billing->setFirst_name ( $row['firstname'] );
			...
			$shipping->setLast_name ( $row['lastname'] );
			$shipping->setFirst_name ( $row['firstname'] );
