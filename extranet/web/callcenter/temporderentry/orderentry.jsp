...
<%
   ...
   String editaction = (String)request.getParameter("editaction");
...

	{
		...
		request.getSession(true).setAttribute("gomanual","");
...
else
{
    ...
    String gomanual = request.getParameter("gomanual");
    if(gomanual == null) {}else{
    if(gomanual.equals("1"))
    {
        request.getSession(true).setAttribute("gomanual","1");
    }
    }
...
if(currorder.coupon_code != null)
{
	...
	if("SUBMIT".equalsIgnoreCase((String)request.getParameter("SUBMIT")))
	{
		...
		else
		{
			...
			} else if("CK".equalsIgnoreCase(currorder.bill_cc_type))
			{
			...
			try
			{
			  currorder.getShippingInfo().setShipOptions(policy.getShippingMethod(currorder,sr,shipOptions).replaceAll("\\ \\(Book\\)","").replaceAll("\\ \\(US\\)","").replaceAll("[()]", ""), "Prepaid", "");
			   ...
			   }else
			   {
                ...
                if(currorder.bill_cc_type.equalsIgnoreCase("CK"))
                {
                    currorder.is_future_ship = 1;
                }
...
%>
<head>
<title><%=client_name%> Order Creation</title>
 <link REL="stylesheet" HREF="/webapps/owd.css" TYPE="text/css">
</head>
<body bgcolor=#FFFFFF>
<CENTER><font face="Geneva, Verdana, Helvetica" size=-1><B><%=client.company_name%> Order Creation (<%= currorder.contactID %>)</B></font></CENTER>
<HR>
...
<font face="Geneva, Verdana, Helvetica" size=-2><B>&nbsp;<%= item.client_part_no %></B></TD><TD ALIGN=LEFT NOWRAP>
...
<font face="Geneva, Verdana, Helvetica" size=-2><B>Product&nbsp;Total:&nbsp;</B></TD>
<TD><font face="Geneva, Verdana, Helvetica" size=-2>
...
<TD ALIGN=RIGHT colspan=2><font face="Geneva, Verdana, Helvetica" size=-2><B>Coupon Discount<%= disctype %>:&nbsp;</B><% if(shippingDiscountApplies ){%><BR>(See ship options below for discount amount)</TD><TD><font face="Geneva, Verdana, Helvetica" size=-2>N/A</TD><%}else{%></TD><TD><font face="Geneva, Verdana, Helvetica" size=-2><%= dform.format(currorder.discount) %></TD><% }%>
</TR>
<TR><TD ALIGN=RIGHT colspan=4><font face="Geneva, Verdana, Helvetica" size=-2><B>Sales Tax:&nbsp;</B></TD>
<TD colspan=1><font face="Geneva, Verdana, Helvetica" size=-2><FORM METHOD=POST ACTION="orderentry.jsp"><%= (policy.allowSalesTaxEditing(currorder)?"<INPUT TYPE=TEXT SIZE=6 NAME=total_tax_cost VALUE=\""+currorder.total_tax_cost+"\">":dform.format(currorder.total_tax_cost))%></TD></TR>
<TR><TD ALIGN=RIGHT colspan=4><font face="Geneva, Verdana, Helvetica" size=-2><B>Shipping/Handling Charge:&nbsp;</B></TD>
<TD colspan=1><font face="Geneva, Verdana, Helvetica" size=-2>
...
<% if (allowShipPriceEditing  && policy.allowShipPriceEditing(currorder)) { %>
 <TR>
<TD ALIGN=RIGHT colspan=4>
<font face="Geneva, Verdana, Helvetica" size=-2><B>Current&nbsp;Order&nbsp;Total:&nbsp;</B></TD>
<TD><font face="Geneva, Verdana, Helvetica" size=-2>
...
<%
	...
	if(currorder.skuList.size()>0 && currorder.getShippingAddress().zip.length()>0)
	{
%>
	<TR><TD width=100% colspan=5><table border=0 width=100%><TR><TD ALIGN=LEFT >
	<font face="Geneva, Verdana, Helvetica" size=-1><B>Shipping&nbsp;Options</B>
	</TD><TD ALIGN=RIGHT width=100%>
	<font face="Geneva, Verdana, Helvetica" size=-2>(Order Total includes discounts and sales tax if they are being automatically included)
	</TD></TR> </table></td></tr>
<TR>
	<TD ALIGN=LEFT colspan=2 >
<FONT  SIZE=-2><b>Method</b>
	</TD>
	<TD ALIGN=LEFT COLSPAN=2>
	<FONT  SIZE=-2><b><%= allowShipPriceEditing?"Estimated Actual Ship Rate":"Shipping Cost" %></b>
	</TD>
    <% if(!allowShipPriceEditing){%>
	<TD ALIGN=LEFT >
	<FONT  SIZE=-2><b>Order&nbsp;Total</b>
	</TD>
...
%>
<TR>
<TD ALIGN=LEFT NOWRAP width=100% COLSPAN=2><INPUT TYPE=RADIO NAME=shipmethod VALUE="<%=  ((ShippingOption) shipOptions.get(i)).code %>" <%= i==0?"CHECKED":"" %>>
<FONT  SIZE=-2><B><%= ((ShippingOption) shipOptions.get(i)).name%></B><BR><%= ((ShippingOption) shipOptions.get(i)).desc%></TD>
<TD ALIGN=LEFT COLSPAN=2><FONT  SIZE=-2><%=formatter.format(((ShippingOption) shipOptions.get(i)).customerCost)%></TD>
    <% if(!allowShipPriceEditing){%>
<TD align=left><FONT  SIZE=-2><b></b>&nbsp;<%= currorder.bill_cc_type.equalsIgnoreCase("FREE")?"$0.00":formatter.format(((((ShippingOption) shipOptions.get(i)).customerCost+currorder.total_product_cost)*(1.00f+currorder.tax_pct))+currorder.discount) %></TD></TR>
<% }
	}else
	{
%>
<TR><TD ALIGN=LEFT COLSPAN=4><INPUT TYPE=RADIO NAME=shipmethod VALUE="<%=  ((ShippingOption) shipOptions.get(i)).code %>">
<FONT  SIZE=-2><B><%= ((ShippingOption) shipOptions.get(i)).name%></B><BR><%= ((ShippingOption) shipOptions.get(i)).desc%>
</TD>
    <% if(!allowShipPriceEditing){%>
<TD align=left><FONT  SIZE=-2>To&nbsp;be&nbsp;determined</TD></TR>
<% } else {%>
<TD align=left><FONT  SIZE=-2></TD></TR><%          }
	}
	}
	}  else {
        %><TR><TD ALIGN=LEFT COLSPAN=5><BR />
	<font face="Geneva, Verdana, Helvetica" size=-1>Shipping Options not available - please set the shipping address and add at least one SKU to see options.
	</TD></TR>
    <TR>
<TD ALIGN=RIGHT colspan=4>
<font face="Geneva, Verdana, Helvetica" size=-2><B>Current&nbsp;Order&nbsp;Total:&nbsp;</B></TD>
...
<TR>
<TD COLSPAN=5 ALIGN=RIGHT>&nbsp;<BR><font face="Geneva, Verdana, Helvetica" size=-1 COLOR=red>
    <p><B><%= ((currorder.bill_cc_type.equals("FLEX") && currorder.total_product_cost>0.00f )?policy.getFlexPayStatement(currorder):"") %></B>
<p><b><%= errorMessage==null?"":errorMessage %>
</b><p>
<p><b><%= warning==null?"":warning %>
</b><p>
