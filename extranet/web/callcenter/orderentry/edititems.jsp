...
*/
	...
	crits.add("i.client_fkey = \""+client_id+"\"");
	crits.add("is_active=1");
    crits.add("f.loc_code=\""+currorder.getFacilityCode()+"\"");
		...
		0,0,
		0,0,0};
		String[] tableColumnDefs = {"i.inventory_num","i.inventory_num",
		"f.qty","price",
		"description",
		"long_desc"};

	String tableFromStmt = "from owd_inventory i join vw_stock_by_facility f on (i.inventory_id = f.inventory_id)";
	...
	int[] tableRenders = {WebTable.kRenderString,WebTable.kRenderString,WebTable.kRenderString,WebTable.kRenderMoney,WebTable.kRenderString, WebTable.kRenderString};
      ...
      int pageNum = ExtranetServlet.getIntegerParam(request,WebTable.kTableShowPage,1);

    if(("Go to Page No.").equals((String)request.getParameter("searchorders")))
    {
       pageNum =   ExtranetServlet.getIntegerParam(request,"goto",ExtranetServlet.getIntegerParam(request,WebTable.kTableShowPage,1));
    }
	...
	if(editactionitems.indexOf("Update") >= 0)
	{
		...
		if(client.getClientPolicy().allowUnitPriceEditing(currorder))
		{
	...
	} else if ("find_items_sku".equals((String)request.getParameter("editactionitems")))
	{
		...
		crits = new ArrayList();
		crits.add("i.inventory_num like \"%"+key+"%\"");
		crits.add("i.client_fkey = \""+client_id+"\"");
        crits.add("f.loc_code=\""+currorder.getFacilityCode()+"\"");
        ...
        table.setDescription("Items containing \""+key+"\" in SKU...");
            ...
            ExtranetServlet.getIntegerParam(request,WebTable.kTableShowPage,-1)==-1)
	{
...
//modified Nov 27
		...
		session.setAttribute("currorder", currorder);
                  ...
                  client.getClientPolicy().updateLineItemsAfterItemChange(currorder);
		try
		{
			request.getRequestDispatcher("orderentry.jsp").forward(request, response);
		}
...
<CENTER><fontx face="Geneva, Verdana, Helvetica" size=-1><B>Editing Line Items</B></font></CENTER>
...
<TD ALIGN=LEFT>
<fontx face="Geneva, Verdana, Helvetica" size=-2><B>&nbsp;<%= (client.getClientPolicy().allowUnitPriceEditing(currorder)?"<INPUT TYPE=TEXT SIZE=5 NAME=item_"+i+"_price VALUE=\""+item.sku_price+"\">":dform.format(item.sku_price))%></B></TD>
<TD ALIGN=LEFT>
