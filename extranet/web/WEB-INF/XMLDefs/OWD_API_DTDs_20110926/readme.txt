...
All requests should begin with the XML preamble element, followed by a newline, followed by the XML data itself beginning with <OWD_API_REQUEST>.
...
Some entities have api calls beginning with OWD_TEST. These calls are only valid for an OWD-flagged test account and are provided to simulate OWD activities for automated testing of clients. Current calls are:

OWD_TEST_INVENTORY_SETCOUNT_REQUEST (in owd_inventory_api.dtd)
