...
https://secure.owd.com/api/api.jsp

For testing, a separate staging system may be accessed, with the same credentials as in production, through:

https://secure.owd.com/test/api/api.jsp

The contents of the staging database will be periodically reset.
...
OWD_ORDER_HOLD_REQUEST.dtd
OWD_ORDER_UPDATE_REQUEST.dtd
OWD_ORDER_STATUS_REQUEST.dtd
OWD_ORDER_SUMMARY_RESPONSE.dtd
OWD_ORDER_STATUS_RESPONSE.dtd

These are the order-related DTDs. TMost calls return an ORDER_STATUS_RESPONSE, but the ORDER_STATUS_REQUEST can optionally return an ORDER_SUMMARY_RESPONSE instead, which just provides details relevant after an order has shipped.
...
OWD_INVOICE_DATA_API.dtd

This file provides functions to view billing information about shipping activity.
...
Any questions can be directed to casetracker@owd.com for assistance.
