...
private final static Logger log =  LogManager.getLogger();
    ...
    final static int kRenderString = 0;
    final static int kRenderMoney = 1;
...
//////log.debug("xx cols="+columns.length);
        ...
        {
//////log.debug("yy"+" "+i);
            table.append("<td bgcolor=\"#649CCC\"><font color=\"#FFFFFF\"><b>");

            if(i>0) table.append("<A HREF=\""+req.getRequestURI()+"?"+params.toString()+(p>0?"&":"")+kTableShowPage+"="+(pageNum)+"&"+kTableSortColumn+"="+(i+1)+"\"><FONT color=yellow><</FONT></A>");
            table.append(columns[i]);
            if(i>0) table.append("<A HREF=\""+req.getRequestURI()+"?"+params.toString()+(p>0?"&":"")+kTableShowPage+"="+(pageNum)+"&"+kTableSortColumn+"="+(-1*(1+i))+"\"><FONT color=yellow>></FONT></A>");
...
//////log.debug("xx");
            while(rs.next())
            {
                ...
                {
                    try{
                    Mailer.sendMailWithAttachment("One World search results","Attached is a comma-delimited file containing the data you requested\r\n\r\n("+description+")",email,message.toString().getBytes(),"results.csv","application/octet-stream");
...
////log.debug("xx");
        ...
        tablenav.append("<HR><B>&nbsp;</B>");

        tablenav.append("<TABLE width=100%><TR><TD width=50% align=left valign=bottom><B>");
        if (pageNum > 1)
            tablenav.append("<B><A HREF=\""+req.getContextPath()+ExtranetServlet.kExtranetURLPath+"?"+params.toString()+(p>0?"&":"")+kTableShowPage+"="+(pageNum-1)+"&"+kTableSortColumn+"="+orderCol+"\">Previous&nbsp;Page</A></B>");
    ...
    tablenav.append("</TD><TD width=50%  align=right valign=bottom><B>");
    tablenav.append("<B><FORM METHOD=POST ACTION=\""+req.getContextPath()+ExtranetServlet.kExtranetURLPath+"\">");
    ...
    tablenav.append("<INPUT TYPE=HIDDEN NAME=\""+kTableSortColumn+"\" VALUE=\""+orderCol+"\">");
        tablenav.append("\n" +
                ...
                "    <INPUT TYPE=\"HIDDEN\" name=\""+ ExtranetServlet.kParamChangeMgrTo+"\" value=\""+ ExtranetServlet.getSessionString(req,ExtranetServlet.kExtranetCurrMgr) +"\" />");
            tablenav.append("<B>Email&nbsp;Results&nbsp;To:&nbsp;<INPUT TYPE=TEXT NAME=tdest SIZE=15><INPUT TYPE=SUBMIT NAME=xxx VALUE=Send></TD></TR></TABLE>");


      //  resp.getWriter().write(tablenav.toString()+table.toString()+"");
        StringBuffer tablenav2 = new StringBuffer();

        tablenav2.append("<TABLE width=100%><TR><TD width=100% align=left valign=bottom><B>");
        if (pageNum > 1)
            tablenav2.append("<B><A HREF=\""+req.getContextPath()+ExtranetServlet.kExtranetURLPath+"?"+params.toString()+(p>0?"&":"")+kTableShowPage+"="+(pageNum-1)+"&"+kTableSortColumn+"="+orderCol+"\">Previous&nbsp;Page</A></B>");

        tablenav2.append("&nbsp;&nbsp;Page&nbsp;"+pageNum+"&nbsp;of&nbsp;"+maxpage+"&nbsp;&nbsp;");
        if (pageNum<maxpage)
            tablenav2.append("<B><A HREF=\""+req.getRequestURI()+"?"+params.toString()+(p>0?"&":"")+kTableShowPage+"="+(pageNum+1)+"&"+kTableSortColumn+"="+orderCol+"\">Next&nbsp;Page</A></B>");
        tablenav2.append("&nbsp;&nbsp;<input type=\"submit\" name=\"searchorders\" value=\"Go to Page No.\"><input type=\"text\" value=\"\" name=\""+kTableGotoPage+"\" size=\"5\">");
        return tablenav.toString()+table.toString()+tablenav2.toString()+"</TD></TR></FORM></TABLE>";
    }
