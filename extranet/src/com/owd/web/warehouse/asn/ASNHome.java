...
private final static Logger log =  LogManager.getLogger();
    ...
    public static String getClientSelector(HttpServletRequest req) {
        ...
        try {
            ...
            if (UserManager.isInternal(UserFactory.getOwdUserFromLogin(user))) {
    ...
    public static final String kASNSearchStatus = "asn.search.status";
    public static final String kASNSearchType = "asn.search.type";
    public static final String kASNSearchString = "asn.search.string";
    public static final String kASNSearchCompare = "asn.search.compare";
    public static final String kASNSearchFacility = "asn.search.facility";


    public static final void setSearchSessionAttributes(HttpServletRequest req) {
        HttpSession wsess = req.getSession(true);
        if (null == wsess.getAttribute(kASNSearchStatus)) {
            wsess.setAttribute(kASNSearchStatus, "-1");
        }
        if (null == wsess.getAttribute(kASNSearchType)) {
            wsess.setAttribute(kASNSearchType, "");
        }
        if (null == wsess.getAttribute(kASNSearchString)) {
            wsess.setAttribute(kASNSearchString, "");
        }
        if (null == wsess.getAttribute(kASNSearchCompare)) {
            wsess.setAttribute(kASNSearchCompare, "0");
        }
        ...
        if (!getSessionFlag(req,kExtranetAuthenticatedFlag) || (!(req.getUserPrincipal().getName().equals(getSessionString(req,kExtranetAuthenticatedUserName))))) {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    setSessionFlag(req, kExtranetAdminFlag, (rs.getInt(2) == 1 && getSessionFlag(req, ASNHome.kExtranetInternalFlag)));
                    if(false==getSessionFlag(req, ASNHome.kExtranetInternalFlag) || "".equals(getSessionString(req, ASNHome.kExtranetClientID)))
                    {
