...
import com.owd.web.batchimporters.OWDWebBatchUploadOrdersData;
 ...
 */
public class OWDUploadOrdersServlet extends HttpServlet {
private final static Logger log =  LogManager.getLogger();
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            if (req.getSession(true).getAttribute("inprogress") == null) {
                    ...
                    if (req.getSession(true).getAttribute("resultsList") == null) {
                        ...
                        if (!files.hasMoreElements()) {
                            ...
                            if (uploadFile == null) {
                                ...
                                resp.sendRedirect("/webapps/clienttools/batchimporters/processprogress.jsp");
                ...
                if (req.getSession(true).getAttribute("inprogress") == null)
                    resp.sendRedirect("/webapps/clienttools/batchimporters/orderuploads.jsp");
            } else {
                resp.sendRedirect("/webapps/clienttools/batchimporters/processprogress.jsp");
            }
        } catch (Exception ex) {
    ...
    public class importProcessorThread implements Runnable {
        ...
        public void run() {
            ...
            OWDWebBatchUploadOrdersData dataHandlerWebBatch = new OWDWebBatchUploadOrdersData();
            dataHandlerWebBatch.init(data);

            theSession.setAttribute("toprocess", "" + dataHandlerWebBatch.getOrderCount());

            for (int orderIndex = 0; orderIndex < dataHandlerWebBatch.getOrderCount(); orderIndex++) {
                try {
                    resultsList.add(processOrder(dataHandlerWebBatch, clientID, backorder_rule, orderIndex));
        ...
        private List processOrder(OWDWebBatchUploadOrdersData dataHandlerWebBatch, String clientID, String backorderRule, int orderIndex) {
            //returns list of two elements - com.owd.api.client Order ID and response
            List response = new ArrayList();
            //add new
            response.add(dataHandlerWebBatch.getOrderReference(orderIndex));
            ...
            try {
                ...
                dataHandlerWebBatch.loadOrder(order, orderIndex);
