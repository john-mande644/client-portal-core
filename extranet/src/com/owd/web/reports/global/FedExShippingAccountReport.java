 ...
 */
public class FedExShippingAccountReport extends ReportDefinition {
private final static Logger log =  LogManager.getLogger();

    List paramList = new ArrayList();
    String prepareSQL = "select company_name as Client, " +
            "shipmentdate as \'Activity Date\'," +
            "invoicedate as \'Invoice Date\'," +
            "trackingNumber as \'Tracking Number\',ISNULL(carr_service,servicetypecode) as \'Ship Method\', numberofpieces as \'Package Count\'," +
            " case when order_fkey is null then 'Inbound' else 'Outbound' end as \'Type\',convert(money,netcharges)+convert(money,incentive) as \'Amount\'," +
            ...
            " originalcustrefnotes as \'Reference1\','' as \'Reference2\'" +
            " from fedex_ebills u (NOLOCK) " +
            ...
            " where needs_review=0 and bill_client=1 and convert(money,netcharges)+convert(money,incentive)-client_incentive-item_value <> 0.00" +
            " and billdate >= ? and billdate < dateadd(day,1,?) and client_id=?";

    public FedExShippingAccountReport() {
